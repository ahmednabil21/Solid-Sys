...
const colorClasses = {
  ...
  teal: {
    surface: 'bg-teal-100/45 dark:bg-teal-900/25',
    icon: 'text-teal-700 dark:text-teal-300',
    border: 'border-teal-200/75 dark:border-teal-700/45',
    glow: 'shadow-teal-200/35 dark:shadow-teal-950/30',
    iconWrap: 'bg-teal-100/70 dark:bg-teal-900/35 ring-1 ring-teal-200/65 dark:ring-teal-700/45'
  }
};
...
export const StatCard: React.FC<StatCardProps> = ({ 
  ...
  const cardClass = glass
    ? `stat-card min-w-0 rounded-2xl border p-4 sm:p-6 lg:p-8 bg-white/30 dark:bg-white/5 ${colors.border} border-white/50 dark:border-white/10 shadow-lg shadow-black/5 dark:shadow-black/20 backdrop-blur-xl backdrop-saturate-150 transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl hover:bg-white/40 dark:hover:bg-white/10`
    : `stat-card min-w-0 rounded-2xl border p-4 sm:p-6 lg:p-8 ${colors.surface} ${colors.border} ${colors.glow} backdrop-blur-md transition-all duration-300 hover:-translate-y-0.5 hover:shadow-xl`;

  const iconWrapClass = glass
    ? `p-2 sm:p-3 lg:p-5 rounded-full bg-white/40 dark:bg-white/10 ring-1 ring-white/50 dark:ring-white/15 backdrop-blur-sm`
    : `p-2 sm:p-3 lg:p-5 rounded-full ${colors.iconWrap}`;
