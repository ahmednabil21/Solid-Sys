...
const UsersPage: React.FC = () => {
  ...
  const handleAddRoleChange = (role: UserRole) => {
    setAddForm((prev) => {
      ...
      if (role === UserRole.Employee) {
        Object.assign(next, DEFAULT_EMPLOYEE_PERMISSIONS, { canAddMaterial: true, canDisburseMaterial: true });
      }
  ...
  const handleCreateUser = (e: React.FormEvent) => {
    ...
    if (addForm.role === UserRole.Employee) {
      ...
      payload.canAddMaterial = addForm.canAddMaterial ?? true;
      payload.canDisburseMaterial = addForm.canDisburseMaterial ?? true;
            ...
            <form onSubmit={handleCreateUser} className="p-6 space-y-4">
              ...
              {addForm.role === UserRole.Employee && (
                <div className="space-y-2 p-3 rounded-lg bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700">
                  ...
                  {(Object.keys(EMPLOYEE_PERMISSION_LABELS) as (keyof typeof EMPLOYEE_PERMISSION_LABELS)[]).map((key) => (
                    <label key={key} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                      <input
                        type="checkbox"
                        checked={!!(addForm as unknown as Record<string, boolean>)[key]}
                  ...
                  <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    <input
                      type="checkbox"
                      checked={!!addForm.canAddMaterial}
                      onChange={(e) => setAddForm((f) => ({ ...f, canAddMaterial: e.target.checked }))}
                      className="rounded border-gray-300"
                    />
                    إضافة مادة للمخزن
                  </label>
                  <label className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                    <input
                      type="checkbox"
                      checked={!!addForm.canDisburseMaterial}
                      onChange={(e) => setAddForm((f) => ({ ...f, canDisburseMaterial: e.target.checked }))}
                      className="rounded border-gray-300"
                    />
                    صرف مواد
                  </label>
